...
function App() {
	...
	return (
		<div
			className="max-h-[100dvh] bg-cover bg-center bg-no-repeat transition-all duration-500 overflow-y-hidden"
			...
			<div className='hidden sm:flex'>
				...
				<main className="relative z-10 min-h-[calc(100vh-6rem)] flex items-center justify-center p-4 overflow-hidden w-full">
					...
					>
						{/* Content Container */}
						<div
							className={`relative flex flex-row h-full transform transition-transform duration-500 ease-out ${sliding ? (direction === 'right' ? 'translate-x-full' : '-translate-x-full') : 'translate-x-0'
								}`}
						>
							{/* Left Content */}
							<div className="w-full lg:w-1/2 p-6 md:p-16 flex flex-col justify-center">
								...
								<div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-1 sm:grid-cols-2 gap-6 mb-6 w-full">
									<div className="flex items-center gap-2 text-white/80">
										<Clock className="w-4 h-4 md:w-6 md:h-6" />
										<div>
											<p className="text-md text-white/60 whitespace-nowrap">Prep Time</p>
									...
									<div className="flex items-center gap-2 text-white/80">
										...
										<div>
											<p className="text-md text-white/60 whitespace-nowrap">Cook Time</p>
							...
							<div className="flex w-full lg:w-1/2 px-10 pb-6 items-center justify-center">
								<img
			...
			<div className='sm:hidden flex flex-col'>
				...
				<main className="relative z-10 min-h-[calc(100vh-6rem)] flex items-center justify-center p-4 overflow-hidden w-full">
					...
					>
						...
						>
							{/* Right Content - Main Image */}
							<div className="flex w-full px-10 py-6 items-center justify-center">
								<img
									src={recipes[currentIndex].image}
									alt={recipes[currentIndex].name}
									className="w-full h-[calc(100-20)%] object-cover rounded-2xl shadow-2xl"
							...
							{/* Left Content */}
							<div className="w-full lg:w-1/2 p-6 md:p-16 flex flex-col justify-center">
								<h2 className="text-base font-bold text-white mb-6 md:mb-8 leading-tight w-full">
									{recipes[currentIndex].name}
								</h2>

								<div className="flex flex-wrap gap-6 mb-6 w-full whitespace-nowrap">
									<div className="flex items-center gap-2 text-white/80">
										<Clock className="w-4 h-4 md:w-6 md:h-6" />
										<div>
											<p className="text-sm text-white/60">Prep Time</p>
											<p className="text-xs md:text-base font-medium">{recipes[currentIndex].prepTime}</p>
										</div>
									</div>
									<div className="flex items-center gap-2 text-white/80">
										<UtensilsCrossed className="w-4 h-4 md:w-6 md:h-6" />
										<div>
											...
											<p className="text-xs font-medium">{recipes[currentIndex].cookTime}</p>
										</div>
									</div>
									<div className="flex items-center gap-2 text-white/80">
										...
										<div>
											...
											<p className="text-xs font-medium">{recipes[currentIndex].servings}</p>
										</div>
									</div>
								</div>
